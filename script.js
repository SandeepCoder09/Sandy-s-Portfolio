...
// Binary background canvas animation
(function () {
  ...
  // Config — tweak for density/speed/color
  const config = {
    fontSize: 18,          // base font size (will scale on high DPI)
    density: 0.035,        // fraction of screen width used to compute columns (smaller = fewer columns)
    speed: 0.6,            // global speed multiplier (0.3 slow, 1 normal, 1.5 faster)
    charColor: '#00ff99',  // binary color (cyber-green)
    opacity: 0.14,         // canvas opacity (also set in CSS)
    fpsLimit: 60           // target FPS limit
  };
  ...
  function resize() {
    ...
    // compute columns
    const colWidth = Math.max(12, config.fontSize);
    cols = Math.floor(w * config.density / (colWidth / 12)) || Math.floor(w / colWidth);
    // create drops (y position per column)
    drops = new Array(cols).fill(0).map(() => Math.random() * h);
  }

  // draw loop
  let lastTime = 0;
  const msPerFrame = 1000 / config.fpsLimit;
  ...
  function draw(time = 0) {
    ...
    // set font based on width for responsiveness
    const fontSize = Math.max(12, Math.round((config.fontSize * (Math.min(w, 1400) / 1400))));
    ctx.font = `${fontSize}px "Space Mono", "Roboto Mono", monospace`;
    ctx.textBaseline = 'top';
    ctx.fillStyle = config.charColor;
    ctx.globalAlpha = 0.9;

    const colGap = Math.max(10, Math.floor(w / cols));

    for (let i = 0; i < cols; i++) {
      const x = i * colGap + (colGap - fontSize) / 2;
      let y = drops[i];

      // draw a few characters per column with slight vertical spacing
      const step = fontSize * (0.9 + Math.random() * 0.6);
      if (y > h + Math.random() * 1000) {
        drops[i] = -Math.random() * 200; // reset above the screen
      } else {
        drops[i] += step * config.speed * (0.5 + Math.random() * 0.8);
      }

      // Draw 0/1 chars down the column, shorter tail for subtle effect
      const tailLen = 3; // small tail for mild effect
      for (let k = 0; k < tailLen; k++) {
        const yy = y - k * step;
        if (yy < 0 || yy > h) continue;
        // fade the tail
        ctx.globalAlpha = Math.max(0.05, 0.9 - k * 0.35);
        // optionally, make head brighter
        const char = chars[Math.floor(Math.random() * chars.length)];
        ctx.fillStyle = k === 0 ? '#bfffd6' : config.charColor;
  ...
  // Optional: pause animation on page hidden to save CPU
  document.addEventListener('visibilitychange', () => {
    if (document.hidden) {
      // stop rendering by setting fpsLimit very low
      // (we keep animation but it will run but produce less frames)
    } else {
